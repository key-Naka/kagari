...
describe('Compose 服务隔离 seam', () => {
  it('仅将 Web 与 API 绑定到回环地址，且数据服务限制在内部网络', async () => {
    ...
    expect(serviceBlock('backend')).toContain('ADMIN_USERNAME: ${ADMIN_USERNAME:?ADMIN_USERNAME is required}')
    expect(serviceBlock('backend')).toContain('ADMIN_PASSWORD: ${ADMIN_PASSWORD:?ADMIN_PASSWORD is required}')
    expect(serviceBlock('backend')).toContain('SESSION_TTL: ${SESSION_TTL:-24h}')
    expect(serviceBlock('backend')).toContain('CORS_ORIGIN: ${CORS_ORIGIN:?CORS_ORIGIN is required}')
    expect(serviceBlock('backend')).toContain('COOKIE_DOMAIN: ${COOKIE_DOMAIN:?COOKIE_DOMAIN is required}')
