...
describe('Track 页面与管理契约', () => {
  ...
  it('管理员页面自动识别媒体元数据，通过受保护接口创建、编辑、启停和排序 Track', async () => {
    ...
    expect(page).toContain('/api/v1/admin/media/upload-credentials')
    expect(page).toContain('/api/v1/admin/media')
    expect(page).toContain('/api/v1/admin/tracks')
    expect(page).toContain("method: activeTrackId.value === null ? 'POST' : 'PUT'")
    expect(page).toContain("audio.addEventListener('loadedmetadata'")
    expect(page).toContain('onMounted(loadTracks)')
    expect(page).toContain('durationMs: Math.round(audio.duration * 1000)')
